...
inline int dsvd(Matrix<double> &a, int m, int n, double *w, Matrix<double> &v);
///////////////////////////////////////////////////////
inline void V2v(Vector<double> &V, vector<double> &v)
{
    v.clear();
...
/*
inline void M2vv(Matrix<double> &M, vector<vector<double>> &vv)
{
    vv.clear();
    for(int i=0;i<M.GetRows();i++)
    {
        vv.push_back(new std::vector<double>());
        for(int j=0;j<M.GetCols();j++)
        {
            vv[i].push_back(M[i][j]);
        }
    }
}
*/
vector<double> SVD(vector<vector<double> > &vv)
{
    Matrix<double> m(vv);
	Vector<double> w(m.GetRows());
	vector<double> ans;
