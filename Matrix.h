...
public:
	...
	Matrix(unsigned int r, unsigned int c, double *data);
...
template<class T>
Matrix<T>::Matrix(unsigned int r, unsigned int c, double *data) :
m_Rows(r),
m_Cols(c),
m_Data(data)
{
...
void matrix_inverse(const double *Min, double *Mout, int actualsize);
	...
	while(!sorted)
	{
		...
		for (unsigned int i=0; i<v.Size()-1; i++)
		{
			if (v[i]<v[i+1])
			{
				...
				Vector<double> rtmp = GetRowVector(i);
	...
	while(!sorted)
	{
		...
		for (unsigned int i=0; i<v.Size()-1; i++)
		{
			if (v[i]<v[i+1])
			{
				...
				Vector<double> rtmp = GetColVector(i);
